...
Shader "Ai/Beginner/11_AdditionalMaps_Fragment"
{
	Properties
	{
	...
	SubShader
	{
		Pass
		{
			...
			float4 frag (vertOut vOut) : COLOR
			{
				...
				float4 texE = tex2D (_EmitMap, _EmitMap_ST.xy * vOut.tex.xy + _EmitMap_ST.zw);
...
//				}
				...
				float3 lightFinal = diffuseReflection + (specularReflection * tex.a) + rimLighting + (texE.xyz * _EmitStrength) + UNITY_LIGHTMODEL_AMBIENT.xyz;
